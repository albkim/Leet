...
/**
 ...
 Maybe use a map & set to see if we see a number, and whether its a match or non-match

 */
public class Bulls_Cows_299 {

    public static String getHint(String secret, String guess) {
        // first find a match since we need to handle dupes
        int length = Math.min(secret.length(), guess.length());

        // reversing the strings make the calc easier
        ...
        int match = 0;
        boolean[] matchFound = new boolean[length];
        for (int index = 0; index < length; index++) {
            if (sbSecret.charAt(index) == sbGuess.charAt(index)) {
        ...
        for (int index = 0; index < sbSecret.length(); index++) {
            if (matchFound[index]) {
                continue;
            }

            char chr = sbSecret.charAt(index);
            lookup.put(chr, lookup.getOrDefault(chr, 0) + 1);
        }

        // now do a count
        int wrongPosition = 0;
        for (int index = 0; index < sbGuess.length(); index++) {
            if (matchFound[index]) {
                continue;
            }

            char chr = sbGuess.charAt(index);

            if (lookup.containsKey(chr)) {
                int count = lookup.get(chr) - 1;
                if (count == 0) {
                    lookup.remove(chr);
                }
                else {
                    lookup.put(chr, count);
                }
                wrongPosition++;
            }
        }
